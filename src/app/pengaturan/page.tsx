...
export default function PengaturanPage() {
  ...
  const hasPrinterChanges = useMemo(() => {
    ...
    return formData.receiptPaperSize !== (currentSettings.receiptPaperSize || DEFAULT_FORM_DATA.receiptPaperSize) ||
           ...
           formData.wifiInstructions !== (currentSettings.wifiInstructions || DEFAULT_FORM_DATA.wifiInstructions);
  }, [formData, currentSettings]);
  ...
  const handleUndo = (type: 'business' | 'printer' | 'theme') => {
    ...
    if (type === 'business') {
        ...
        setFormData(prev => ({
            ...
            wifiInstructions: currentSettings.wifiInstructions || DEFAULT_FORM_DATA.wifiInstructions,
        }));
    } else if (type === 'theme') {
        setFormData(prev => ({
            ...prev,
    ...
    <div className="flex flex-col gap-8 pb-20">
      ...
      <Tabs defaultValue="business" className="w-full">
        <TabsList className="bg-muted/50 p-1.5 h-14 rounded-[1.5rem] mb-8 border flex w-full sm:w-fit overflow-x-auto shadow-inner">
            ...
            <TabsTrigger value="printer" className="rounded-xl font-black uppercase text-[10px] tracking-widest gap-2 data-[state=active]:bg-background data-[state=active]:text-primary px-6"><Printer className="size-3.5" /> Printer & Nota</TabsTrigger>
        ...
        <TabsContent value="business" className="animate-in fade-in slide-in-from-bottom-2">
            <Card className="rounded-[2rem] overflow-hidden border-border shadow-sm max-w-3xl">
                ...
                <CardFooter className="bg-muted/10 border-t p-4 flex justify-end gap-3">
                    <Button variant="ghost" size="sm" onClick={() => handleUndo('business')} disabled={!hasBusinessChanges || isSaving} className="font-bold text-[10px] uppercase gap-2"><RotateCcw className="size-3" /> Urungkan</Button>
                    <Button size="sm" onClick={handleSave} disabled={!hasBusinessChanges || isSaving} className="font-black uppercase text-[10px] tracking-widest px-8 rounded-xl shadow-lg shadow-primary/20">{isSaving ? 'Menyimpan...' : 'Simpan Profil'}</Button>
                </CardFooter>
            </Card>
        </TabsContent>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                <div className="lg:col-span-7 space-y-6">
                    <Card className="rounded-[2rem] border-primary/20 bg-primary/[0.01] overflow-hidden shadow-sm">
                        <CardHeader className="bg-primary/5 border-b border-primary/10"><CardTitle className="text-lg font-black uppercase tracking-tight">Konfigurasi Struk</CardTitle></CardHeader>
                        <CardContent className="space-y-6 pt-8">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="space-y-3">
                                    <Label className="text-[10px] font-black uppercase text-muted-foreground">Lebar Kertas</Label>
                                    <Select value={formData.receiptPaperSize} onValueChange={(val) => setFormData({...formData, receiptPaperSize: val})}><SelectTrigger className="h-12 bg-background rounded-xl font-bold"><SelectValue /></SelectTrigger><SelectContent className="rounded-xl"><SelectItem value="58mm">58mm</SelectItem><SelectItem value="80mm">80mm</SelectItem></SelectContent></Select>
                                </div>
                                <div className="space-y-3">
                                    <Label className="text-[10px] font-black uppercase text-muted-foreground">Ukuran Font (px)</Label>
                                    <Input type="number" value={formData.receiptFontSize} onChange={(e) => setFormData({...formData, receiptFontSize: parseInt(e.target.value) || 12})} className="h-12 rounded-xl bg-background font-bold" />
                                </div>
                                <div className="space-y-3">
                                    <Label className="text-[10px] font-black uppercase text-muted-foreground">Ketebalan Font</Label>
                                    <Select value={formData.receiptFontWeight} onValueChange={(val) => setFormData({...formData, receiptFontWeight: val})}><SelectTrigger className="h-12 bg-background rounded-xl font-bold"><SelectValue /></SelectTrigger><SelectContent className="rounded-xl">{FONT_WEIGHT_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value} className="font-bold">{opt.label}</SelectItem>)}</SelectContent></Select>
                                </div>
                            </div>
                            <div className="space-y-3">
                                <Label className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2"><Wifi className="size-3 text-primary" /> Panduan Koneksi Wi-Fi (Untuk Nota Wi-Fi)</Label>
                                <Textarea value={formData.wifiInstructions} onChange={(e) => setFormData({...formData, wifiInstructions: e.target.value})} placeholder="Contoh: 1. Konekan ke SSID XENON..." className="min-h-[100px] bg-background border-primary/20 rounded-xl font-bold py-4 text-xs leading-relaxed" />
                                <p className="text-[9px] text-muted-foreground italic">Panduan ini hanya muncul saat mencetak struk transaksi kupon Wi-Fi.</p>
                            </div>
                            <div className="space-y-3">
                                <Label className="text-[10px] font-black uppercase text-muted-foreground">Pesan Footer (Penutup)</Label>
                                <Textarea value={formData.receiptFooter} onChange={(e) => setFormData({...formData, receiptFooter: e.target.value})} className="min-h-[80px] bg-background rounded-xl font-bold py-4 text-xs leading-relaxed" />
                            </div>
                        </CardContent>
                        <CardFooter className="bg-muted/10 border-t p-4 flex justify-end gap-3">
                            <Button variant="ghost" size="sm" onClick={() => handleUndo('printer')} disabled={!hasPrinterChanges || isSaving} className="font-bold text-[10px] uppercase gap-2"><RotateCcw className="size-3" /> Urungkan</Button>
                            <Button size="sm" onClick={handleSave} disabled={!hasPrinterChanges || isSaving} className="font-black uppercase text-[10px] tracking-widest px-8 rounded-xl shadow-lg shadow-primary/20">{isSaving ? 'Menyimpan...' : 'Simpan Layout'}</Button>
                        </CardFooter>
                    </Card>
                </div>
                ...
                <div className="lg:col-span-5 sticky top-24">
                    <Card className="rounded-[2.5rem] overflow-hidden border-border bg-muted/10 shadow-2xl">
                        <CardHeader className="bg-card border-b py-4">
                            <div className="flex items-center gap-2"><Eye className="size-4 text-primary" /><CardTitle className="text-sm font-black uppercase tracking-widest text-primary">Live Preview Kupon Wifi</CardTitle></div>
                        </CardHeader>
                        <CardContent className="p-8 flex justify-center bg-slate-200/50">
                            <div className={cn("bg-white text-black shadow-2xl p-5 font-mono leading-tight transition-all duration-500", formData.receiptPaperSize === '58mm' ? "w-[220px]" : "w-[300px]")} style={{ minHeight: '400px', fontSize: `${(formData.receiptFontSize || 12) * 0.7}px`, fontWeight: formData.receiptFontWeight || '500' }}>
                                <div className="text-center mb-4"><div className="uppercase leading-none" style={{ fontSize: '1.2em', fontWeight: 'bold' }}>{formData.storeName || 'XENONPLAY'}</div></div>
                                <div className="border-t border-dashed border-black/30 my-2" />
                                <div className="text-center py-2"><span className="text-[10px] font-black uppercase opacity-60">Kode Kupon Wi-Fi</span><div className="text-3xl font-black tracking-widest my-2">72A9X</div></div>
                                <div className="border-t border-dashed border-black/30 my-2" />
                                <div className="space-y-2 mt-4"><span className="text-[9px] font-black uppercase border-b border-black inline-block">Cara Konek:</span><div className="whitespace-pre-wrap leading-relaxed opacity-80" style={{ fontSize: '0.9em' }}>{formData.wifiInstructions}</div></div>
                                <div className="border-t border-dashed border-black/30 my-4" />
                                <div className="text-center whitespace-pre-wrap leading-relaxed italic opacity-60" style={{ fontSize: '0.8em' }}>{formData.receiptFooter}</div>
                            </div>
                        </CardContent>
                    </Card>
                </div>
        ...
        <TabsContent value="theme" className="animate-in fade-in slide-in-from-bottom-2">
            <Card className="rounded-[2rem] border-border overflow-hidden shadow-sm max-w-3xl">
                ...
                <CardFooter className="bg-muted/10 border-t p-4 flex justify-end gap-3">
                    <Button variant="ghost" size="sm" onClick={() => handleUndo('theme')} disabled={!hasThemeChanges || isSaving} className="font-bold text-[10px] uppercase gap-2"><RotateCcw className="size-3" /> Urungkan</Button>
                    <Button size="sm" onClick={handleSave} disabled={!hasThemeChanges || isSaving} className="font-black uppercase text-[10px] tracking-widest px-8 rounded-xl shadow-lg shadow-primary/20">{isSaving ? 'Menyimpan...' : 'Simpan Tema'}</Button>
                </CardFooter>
            </Card>
        </TabsContent>
